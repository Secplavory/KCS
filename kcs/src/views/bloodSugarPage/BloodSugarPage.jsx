...
function BloodSugarPage(props) {
  ...
  const getUserBloodSugar = useCallback(async () => {
    try {
      const result = appAxios.get(
        `/getUserBloodSugar?userId=${props.userId}&page=1&limit=15`
      );
  ...
  const saveModify = async () => {
    try {
      ...
      const targetData = userBloodSugarData.filter(
        (a) => a.sugarId === modifySugarId
      )[0];
            ...
            <tbody>
              {userBloodSugarData.map((Element) => (
                <tr
                  key={Element.sugarId}
                  onClick={(e) => {
                    openModifyPage(e, Element.sugarId);
                  }}
                >

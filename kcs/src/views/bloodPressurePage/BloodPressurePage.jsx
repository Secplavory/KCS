...
function BloodPressurePage(props) {
  ...
  const getUserBloodPressure = useCallback(async () => {
    try {
      const result = appAxios.get(
        `/getUserBloodPressure?userId=${props.userId}&page=1&limit=15`
      );
  ...
  const saveModify = async () => {
    try {
      ...
      const targetData = userBloodPressureData.filter(
        (a) => a.pressureId === modifyPressureId
      )[0];
            ...
            <input
              ...
              onChange={(e) => {
                if (e.target.value.length === 3) {
                  document.getElementById('input_second').focus();
                }
              }}
            ></input>
          </div>
            ...
            <input
              ...
              onChange={(e) => {
                if (e.target.value.length === 2) {
                  document.getElementById('input_third').focus();
                }
              }}
            ></input>
          </div>
          ...
          <table>
            ...
            <tbody>
              {userBloodPressureData.map((Element) => (
                <tr
                  key={Element.pressureId}
                  onClick={(e) => {
                    openModifyPage(e, Element.pressureId);
                  }}
                >
      ...
      <div className="BP_modify_record">
        <div className="record_detail">
          ...
          <div className="buttons">
            ...
            <button className="complete" onClick={saveModify}>
              修改完成
            </button>
          </div>
        </div>
